...
def setup_logger():
    ...
    logger.info(
        f'Setup erfolgreich. Laufe mit den folgenden Einstellungen: {SETTINGS}')
...
def check_input(from_file, expectation):
    ...
    if not Path(from_file).is_file():
        is_not_file = f'Suche soll in ("{from_file}") stattinden. Aber das ist keine Datei/Verzeichnis.'
        raise Exception(is_not_file)
    if not isinstance(expectation, int):
        is_not_number = f'Der angegebene Wert erwarteter Sequenzen ("{expectation}") ist keine Zahl.'
    ...
    def get_expectation(file_name):
        ...
        if '-' in file_name:
            try:
                ...
                logger.debug(
                    f'Fehler beim Auslesen von Erwartung aus Dateiname, Fehler: {non_critical_error}')
                print(f'''Aus der Datei {in_file_name} konnte keine Erwartung gelesen werden.
                    Hinweis:
                    * Namenskonvention: <Erwartung>-abc.endung
                    * Beispiel: 3-W0543125.mp3 (Erwartung: 3 Sequenzen)')''')
                print('Durchsuche diese Datei ohne Erwartung an eine Anzahl von Sequenzen.')
                return 0
            return expectation
        return 0
